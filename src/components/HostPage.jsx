...
import { createInvitationOnServer, parseGuestList } from '../invitationLogic.js';
...
export function HostPage() {
  ...
  const handleGenerateLink = async () => {
    ...
    try {
      ...
      // Tải file JSON về máy Host như một bản sao lưu
    //   downloadConfigFile(invitationId, config);

      // Link gửi cho Guest: trang nhập mã chung (mọi người dùng chung link này)
      const url = `${baseUrl}/`;
  ...
  const handleImportFile = (e) => {
    ...
    reader.onload = () => {
      try {
        ...
        const importedId = data.id || Math.random().toString(36).slice(2, 8);
        ...
        createInvitationOnServer(config)
          .then(() => {
            ...
            setGeneratedLink(url);
            alert(
              'Đã nhập thiệp từ file JSON và lưu lại trên server. Bạn có thể dùng trang nhập mã chung để gửi cho khách.'
            );
          })
          .catch(() => {
            const url = `${baseUrl}/`;
            setGeneratedLink(url);
            alert(
              'Đã đọc file JSON nhưng không lưu được lên server. Bạn vẫn có thể dùng trang nhập mã chung để gửi cho khách (nếu thiệp đã tồn tại trên server).'
            );
          });
      } catch {
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      alert('Đã copy link thiệp vào clipboard.');
    } catch {
    ...
    <div className="app-shell">
      ...
      <div className="card">
        ...
        <small>
          ...
          khách. Sau đó nhấn "Generate link" để lấy link gửi cho khách.
        </small>
        ...
        <div className="grid">
          <div>
            <div className="field">
              ...
              <small>
                Mỗi template có số trang & phong cách hiển thị khác nhau.
              </small>
            </div>
            ...
            <div className="field">
              <label htmlFor="imageUrl">Link ảnh (tùy chọn)</label>
              <input
                id="imageUrl"
                type="text"
              ...
              />
              <small>
                Nếu nhập link ảnh hợp lệ, ảnh sẽ hiển thị ở đầu thiệp (có thể là
                ảnh chủ nhân buổi tiệc, ảnh minh hoạ, v.v.).
              </small>
            </div>
...
VIP01 - Gia đình C`}
              ...
              />
              <small>
                Mỗi dòng một khách, có thể dùng dấu <strong>-</strong>,{' '}
                <strong>|</strong> hoặc khoảng trắng để ngăn cách mã và
                tên. Khách có thể nhập <strong>mã</strong> hoặc{' '}
            ...
            {parsedGuests.length > 0 && (
              <div className="field">
                ...
                <small>
                  Nếu bạn có ảnh riêng cho từng khách, hãy nhập link ảnh tương
                  ứng. Nếu bỏ trống, thiệp sẽ dùng ảnh chung (nếu có) hoặc
                  không hiển thị ảnh.
                </small>
                {parsedGuests.map((g) => (
                  <div
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={handleGuestImageChange(g.id)}
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="btn-row">
              <button className="btn" type="button" onClick={handleGenerateLink}>
                Generate link
              </button>
              <span className="note">
                Link sinh ra sẽ dùng trang nhập mã chung{' '}
                <span className="inline-code">/</span>. Khách nhập số thứ tự để
                xem thiệp cá nhân của mình.
              </span>
            </div>

            <div className="field" style={{ marginTop: 12 }}>
              <label htmlFor="importJson">Hoặc nhập thiệp từ file JSON</label>
              <input
                id="importJson"
                type="file"
              ...
              />
              <small>
                Chọn file <code>invitation-*.json</code> đã tải trước đó để
                khôi phục lại cấu hình thiệp.
              </small>
            </div>
