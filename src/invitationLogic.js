...
export function findGuestByInput(rawInput, parsedGuests) {
  ...
  return (
    parsedGuests.find((g) => g.idLower === value) || null
  );
}
...
// Tra cứu mã mời → trả về { invitationId, guestId }
export async function lookupCodeOnServer(code) {
  const res = await fetch(`${API_BASE}/api/invitations/by-code/${encodeURIComponent(code)}`);
  if (res.status === 404) {
    return null;
  }
  if (!res.ok) {
